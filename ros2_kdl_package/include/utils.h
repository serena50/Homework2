...
inline Eigen::Vector3d computeEulerAngles(const KDL::Rotation& R)
{
...
inline Eigen::Vector3d computeEulerRates(const Eigen::Vector3d& euler, const Eigen::Vector3d& omega) 
{
    double phi = euler[0], theta = euler[1];
    Eigen::Matrix3d W;
    W << 0, -sin(phi), cos(phi)*sin(theta),
